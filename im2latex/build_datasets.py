...
from .preprocessor import Nav2TexHFDataset, Nav2TexParquetDataset, Nav2TexFlatParquetDataset
...
def build_datasets(args, data_source: str, tokenizer):
    ...
    if data_path and Path(data_path).exists():
        ...
        if train_dir.exists():
            ...
            if val_dir.exists():
                val_ds = Nav2TexFlatParquetDataset(
    ...
    return (
        Nav2TexHFDataset(data_source, "train", tokenizer, args,
                          names=sources, weights=weights),
        Nav2TexHFDataset(data_source, "validation", tokenizer, args,),
    )
